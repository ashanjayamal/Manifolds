...
\protect\hypertarget{exm:unnamed-chunk-10}{}\label{exm:unnamed-chunk-10}We consider \(M := \{ (y_1, y_2, y_3) \in \mathbb{R}^3 | (y_1)^2 = (y_2)^2 + (y_3)^2 \}\), the double cone.
...
Without losss of generality assume \(V = B_r(x(0))\) with \(r > 0\). Choose \(q_1, q_2 \in U\) with \(q_1^1 > 0\) and \(q_1^2 < 0\). Furthermore, choose a continuous path \(c : [0, 1] \rightarrow V\) with \(c(0) = x(q_1)\), \(c(1) = x(q_2)\) and \(c(t) \neq x(0)\) for all \(t \in [0, 1]\).
